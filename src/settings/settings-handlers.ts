...
import {
  ...
  SETTINGS_RESET_SUGGESTION_CANCELLED_MESSAGE,
  SETTINGS_RESET_SUGGESTION_DONE_MESSAGE,
  SETTINGS_RESET_SUGGESTION_WARNING_MESSAGE,
  SETTINGS_RESET_SUGGESTION_REQUESTS_CLEARED,
  SETTINGS_RESET_SUGGESTION_BLOCKS_CLEARED,
  SETTINGS_PAUSE_DONE_CALLBACK,
  SETTINGS_RESUME_DONE_CALLBACK,
  SETTINGS_BLOCK_LIST_EMPTY_CALLBACK,
  SETTINGS_RESET_SUGGESTION_EMPTY_CALLBACK,
} from "../i18n/settings";
...
} from "../suggestions/pair-history";
import { SETTINGS_CALLBACK } from "./settings-constants";
import {
...
const resetMatchHistoryForUser = async (
  ...
  if (cleared.requests > 0) {
    detailLines.push(
      SETTINGS_RESET_SUGGESTION_REQUESTS_CLEARED.replace(
        "COUNT",
  ...
  if (cleared.blocks > 0) {
    detailLines.push(
      SETTINGS_RESET_SUGGESTION_BLOCKS_CLEARED.replace(
        "COUNT",
  ...
  await ctx.reply(
    SETTINGS_RESET_SUGGESTION_DONE_MESSAGE.replace("DETAIL_LINES", detail),
    withHtml({ reply_markup: mainMenu })
  );
  await renderSettingsHome(ctx, user);
};
...
export const handleSettingsCallback = async (
  ...
  try {
    ...
    if (data === SETTINGS_CALLBACK.resetMatch) {
      ...
      await renderScreen(ctx, {
        text: SETTINGS_RESET_SUGGESTION_WARNING_MESSAGE.replace(
          "REQUEST_COUNT",
